...
class ParamMaxHint:
    ...
    def update(self, g):
        ...
        # Incorporate grad bound
        gradnorm = np.max(np.abs(g))
        ht = np.sqrt(self.d)*self.h
        gtrunc = g if gradnorm < ht else g*ht / (gradnorm + self.eps)
        self.h = max(self.h, gradnorm)
        ...
        self.A += m**2
        self.beta = max(
            min(self.beta - 2.0*m / ((2.0-np.log(3.0))*self.A + self.eps), 0.5 / (ht + self.eps)),
            -0.5 / (ht + self.eps)
        )
